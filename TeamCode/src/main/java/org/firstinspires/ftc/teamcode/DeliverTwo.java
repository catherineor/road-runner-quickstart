...
@Autonomous(name="Deliver Two", group="Linear Opmode")

public class DeliverTwo extends LinearOpMode {
    public State state;
    ...
    public void encoderForwards(int inches, double power, float time){
        ElapsedTime driveTime = new ElapsedTime();
        final double WHEEL_DIAMETER = (7.5/2.54);
        final double COUNTS_PER_INCH = 537.6 / (Math.PI * WHEEL_DIAMETER);
        final int STRAIGHT_COUNTS = (int) (COUNTS_PER_INCH*inches);

        while (Math.abs(driveBackRight.getCurrentPosition()) < Math.abs(STRAIGHT_COUNTS) && driveTime.seconds() < time){
            drive(power);
            telemetry.addData("STRAIGHT_COUNTS",STRAIGHT_COUNTS);
            telemetry.addData("POSITION", driveBackRight.getCurrentPosition());
            telemetry.update();
    ...
    public void encoderCrab(int inches, double power, float time){
        final double WHEEL_DIAMETER = (7.5/2.54);
        final double COUNTS_PER_INCH = 537.6 / (Math.PI * WHEEL_DIAMETER);
        final int STRAIGHT_COUNTS = (int) (COUNTS_PER_INCH*inches);
        ElapsedTime driveTime = new ElapsedTime();
    ...
    public void encoderBackwards(int inches, double power, double time){
        final double WHEEL_DIAMETER = (7.5/2.54);
        final double COUNTS_PER_INCH = 537.6 / (Math.PI * WHEEL_DIAMETER);
        final int STRAIGHT_COUNTS = (int) (COUNTS_PER_INCH*inches*-1);
        ElapsedTime driveTime = new ElapsedTime();

        while (driveBackRight.getCurrentPosition() > STRAIGHT_COUNTS && driveTime.seconds() < time){
            drive(-power);
            telemetry.addData("STRAIGHT_COUNTS",STRAIGHT_COUNTS);
            telemetry.addData("POSITION", driveBackRight.getCurrentPosition());
            telemetry.update();
